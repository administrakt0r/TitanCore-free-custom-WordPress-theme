...
= 1.0.8 =
* Performance: Added static per-request runtime caching to `titancore_get_estimated_reading_time()` and multibyte character calculation fallback for non-Latin script content.
* Performance: Added static per-request runtime caching to `titancore_get_top_tags()` and `titancore_get_published_posts_count()` to eliminate redundant transient database queries.
* Performance: Added static per-request runtime caching to `titancore_get_meta_description()` to prevent duplicate post content parsing and trimming across `wp_head` meta and social tag generators.
