...
function titancore_customize_register( $wp_customize ) {
    ...
    // SEO & Social Section
    $wp_customize->add_section( 'seo_social_options', array(
        'title'    => __( 'SEO & Social', 'titancore' ),
        'priority' => 42,
    ) );

    // Default Open Graph image (fallback when a view has no image of its own)
    $wp_customize->add_setting( 'og_default_image', array(
        'default'           => 0,
        'sanitize_callback' => 'absint',
    ) );

    $wp_customize->add_control( new WP_Customize_Media_Control( $wp_customize, 'og_default_image', array(
        'label'       => __( 'Default Open Graph Image', 'titancore' ),
        'description' => __( 'Used when a page has no featured image. Falls back to the Site Icon if empty.', 'titancore' ),
        'section'     => 'seo_social_options',
        'mime_type'   => 'image',
    ) ) );

    // Social profiles (emitted as sameAs in the Organization schema)
    $social_profiles = array(
        'facebook'  => __( 'Facebook URL', 'titancore' ),
        'x'         => __( 'X (Twitter) URL', 'titancore' ),
        'instagram' => __( 'Instagram URL', 'titancore' ),
        'youtube'   => __( 'YouTube URL', 'titancore' ),
        'linkedin'  => __( 'LinkedIn URL', 'titancore' ),
    );

    foreach ( $social_profiles as $social_network => $social_label ) {
        $wp_customize->add_setting( "social_profile_{$social_network}", array(
            'default'           => '',
            'sanitize_callback' => 'esc_url_raw',
        ) );

        $wp_customize->add_control( "social_profile_{$social_network}", array(
            'label'       => $social_label,
            'description' => __( 'Included as sameAs in the Organization schema on the front page.', 'titancore' ),
            'section'     => 'seo_social_options',
            'type'        => 'url',
        ) );
    }
