...
function titancore_scripts() {
	...
	// TOC active-section highlighting lives in navigation.js, so the script must
	// also load on singular views with the TOC enabled even without a nav menu.
	$should_load_navigation = has_nav_menu( 'primary' )
		|| 'switch' === $theme_mode
		|| ( is_singular() && get_theme_mod( 'show_toc', true ) );
